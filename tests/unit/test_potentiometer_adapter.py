...
def make_adapter(pin=34, min_reading=0, max_reading=65535):
    return PotentiometerAdapter(
        pin=pin, min_reading=min_reading, max_reading=max_reading
    )
...
def test_setup_marks_available_and_configures_attenuation(mocker):
    ...
    adapter = make_adapter(pin=34)
    adapter.setup()

    assert adapter.available is True
    mock_pin_cls.assert_called_once_with(34)
